  ...
  private def assertWithout(
    ...
    Prop.forAll(genMemberInfo) { mi: MemberInfo =>
      Prop.collect(mi.fullName){
        ...
        val miMethodsRemoved = withMethodsRemoved(miMethods).map(show.MethodName(_))
        val expectedMethods  = miMethods.filterNot(methodsToRemove.contains).map(show.MethodName(_))
